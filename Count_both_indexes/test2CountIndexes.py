...
The input fileName is currently: testCountMyIndexes.fastq.gz
...
I have this set to print the 50 most common indexes, but you can change that to any number you want [index = barcode]
"""
...
print
print "You are now searching the file testCountMyIndexes.fastq.gz"
print

#Change the Undetermined fileName 2 lines below this line!!!
barcode_combo_count = Counter()
with gzip.open('testCountMyIndexes.fastq.gz', 'rb') as infile:
    for line in infile:
        if line.startswith('@'):
            ls =line.strip().split(' ')
            barcodes = ls[-1]
...
# get a subset of reads matching some index pattern [commented out by TCG because I'm not using this option]
# assumes dual-indexes [TCG note: this still seems to work OK with single indexes]
# new_counter = Counter({i:j for i,j in barcode_combo_count.iteritems() if "+AGATCTCG" in i})

#More TCG comments and output for idiots like TCG --

#print the total count of all reads - comment the next line if you don't want to see it

print "Here is the sum of all sequences in the file with indexes:"
print sum(barcode_combo_count.values())
print

#print the most common 50 indexes - comment the next line if you don't want to see it
print "Here are the 50 most common indexes in the file:"
print
