...
The input fileName is currently: countMyIndexes.fastq.gz
...
print
print "You are now searching the file countMyIndexes.fastq.gz"
print

#Change the Undetermined fileName 2 lines below this line!!!
barcode_combo_count = Counter()
with gzip.open('countMyIndexes.fastq.gz', 'rb') as infile:
    for line in infile:
        if line.startswith('@'):
            ls =line.strip().split(' ')
            barcodes = ls[-1]
